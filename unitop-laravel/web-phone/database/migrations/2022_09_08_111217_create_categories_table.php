...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('categories', function (Blueprint $table) {
            $table->increments('id');
            $table->string('c_name')->uinque();
            $table->string('c_slug')->default(1)->index();
            $table->char('c_icon')->nullable();
            $table->string('c_avatar')->nullable();
            $table->tinyInteger('c_active')->default(1)->index();
            $table->integer('c_total_product')->default(0); //biết được trong mục đó có bao nhiêu sản phẩm ví dụ laptop(14)
